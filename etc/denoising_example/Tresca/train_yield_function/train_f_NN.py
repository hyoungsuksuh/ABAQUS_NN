...
if denoised:
  data = pd.read_csv("./"+file_name+"_training_data_without_noise.csv")
else:
  data = pd.read_csv("./"+file_name+"_training_data_with_noise.csv")
...
# Define NN input
INPUT = np.concatenate((p_level_set, rho_level_set, theta_level_set), axis=1)
INPUT_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1))
INPUT_scaled = INPUT_scaler.fit_transform(INPUT)
if denoised:
  joblib.dump(INPUT_scaler, "./"+file_name+"_without_noise_f_INPUT_scaler.pkl")
else:
  joblib.dump(INPUT_scaler, "./"+file_name+"_with_noise_f_INPUT_scaler.pkl")
...
if denoised:
  joblib.dump(OUTPUT_scaler, "./"+file_name+"_without_noise_f_OUTPUT_scaler.pkl")
else:
  joblib.dump(OUTPUT_scaler, "./"+file_name+"_with_noise_f_OUTPUT_scaler.pkl")
...
class SB_trainer():
  ...
  # Initialize trainer
  def __init__(self, sINPUT, sOUTPUT, INPUT_scaler, OUTPUT_scaler, SPLIT, WIDTH, EPOCH, BATCH, LR, sobolev, denoised):
    ...
    # Denoised?
    self.denoised = denoised # 0: false; 1: true
  ...
  def save(self, t):
    if t % 10 == 0:
      if self.denoised:
        torch.save(self.model, "./"+file_name+"_without_noise_f_NN.pth")
      else:
        torch.save(self.model, "./"+file_name+"_with_noise_f_NN.pth")
    ...
    if t % 10 == 0:
      ...
      if self.denoised:
        output_history.to_csv("./"+file_name+"_without_noise_f_training_history.csv", index = False)
      else:
        output_history.to_csv("./"+file_name+"_with_noise_f_training_history.csv", index = False)
